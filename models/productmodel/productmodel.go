...
func Detail(id int) entities.Product {
	row := config.DB.QueryRowContext(context.Background(), "SELECT products.id, products.name, categories.name as category_name, products.stock,	products.description, products.created_at, products.updated_at FROM products JOIN categories ON products.category_id = categories.id WHERE products.id = ?", []any{id}...)
